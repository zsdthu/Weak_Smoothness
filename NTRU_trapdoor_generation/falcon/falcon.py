...
from fft import fft, ifft, sub, neg, add_fft, mul_fft,div,mul,adj,add
from ntt import sub_zq, mul_zq, div_zq
from ffsampling import gram, ffldl_fft, ffsampling_fft,ffldl
...
class SecretKey:
    ...
    def __init__(self, n, polys=None):
        ...
        # print(add(mul(self.f,adj(self.f)),mul(self.g,adj(self.g))))
        # print( add(mul(self.F,adj(self.f)),mul(self.G,adj(self.g))))
        # print( div(add(mul(self.F,adj(self.f)),mul(self.G,adj(self.g))),add(mul(self.f,adj(self.f)),mul(self.g,adj(self.g))) ))
        # print("L = ", div(G0[1][0], G0[0][0]))

        # self.T_fft = ffldl_fft(G0_fft)
        # print(self.T_fft)
        # print(len(self.T_fft))
        # print(self.T_fft[0])
        # print(len(self.T_fft[0]))
        # print(self.T_fft[1])
        # print(len(self.T_fft[1]))
        # print(self.T_fft[2])
        # print(len(self.T_fft[2]))
    ...
    def sample_preimage(self, point, seed=None):
        ...
        if seed is None:
            ...
            z_fft, aleas = ffsampling_fft(t_fft, self.T_fft, self.sigmin, urandom)
        else:
            # If a seed is defined, initialize a ChaCha20 PRG
            # that is used to generate pseudo-randomness.
            chacha_prng = ChaCha20(seed)
            z_fft, aleas = ffsampling_fft(t_fft, self.T_fft, self.sigmin,
    ...
    def sign(self, message, randombytes=urandom):
        ...
        while(1):
            if (randombytes == urandom):
                s, aleas = self.sample_preimage(hashed)
            else:
