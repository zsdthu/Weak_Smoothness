...
def gs_norm(f, g, q):
    ...
    # print("norm_fg = ", sqnorm_fg)
    ffgg = add(mul(f, adj(f)), mul(g, adj(g)))
    # print("ffgg = ", ffgg)
    # print("normffgg = ", numpy.linalg.norm(ffgg) ** 2)
    # nn = len(ffgg)
    # Inn = [0] * nn
    # Inn[0] = 1
    # ffgg_inv = div(Inn,ffgg)
    # print("normffgg_inv = ", numpy.linalg.norm(ffgg_inv) ** 2)
    # print("approxi = ", (numpy.linalg.norm(ffgg) * numpy.linalg.norm(ffgg_inv)) ** 2)
    # print(ffgg[0])
    # print(ffgg_inv[0])
    # print("approxi = ", (ffgg[0] * ffgg_inv[0]))
    Ft = div(adj(g), ffgg)
    Gt = div(adj(f), ffgg)
...
def gen_poly(n):
    ...
    assert(n < number)
    f0 = [samplerz(0, sigma, sigma - 0.001) for _ in range(number)]
    f = [0] * n
    k = number // n
    for i in range(n):
